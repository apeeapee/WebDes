...
@section('page_title', 'Kelola Sejarah Desa')
@section('page_subtitle', 'Kelola rentang masa sejarah pembangunan Desa Banyuurip yang tampil di profil desa')

@section('content')
<div class="space-y-6" x-data="{ 
    showAddModal: false, 
    showEditModal: false, 
    editItem: { id: '', tahun: '', judul: '', deskripsi: '' },
    openEdit(item) {
        this.editItem = { ...item };
            ...
            <table class="w-full text-left text-sm text-slate-500">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 font-bold text-slate-850 whitespace-nowrap">
                            {{ $item->judul }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex gap-2">
                                ...
                                <form action="{{ route('admin.sejarah.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus catatan sejarah ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto" x-show="showAddModal" x-cloak>
        <div class="flex min-h-screen items-center justify-center p-4 text-center">
            ...
            <div class="relative w-full max-w-xl rounded-2xl bg-white p-8 text-left shadow-xl border border-slate-100 transform transition-all">
                ...
                <form action="{{ route('admin.sejarah.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Tahun Peristiwa</label>
                        <input type="text" name="tahun" required placeholder="Contoh: 1830 atau 1945" class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm focus:border-brand-green focus:outline-none focus:ring-1 focus:ring-brand-green">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Judul Peristiwa</label>
                        <input type="text" name="judul" required placeholder="Contoh: Asal Usul Nama Desa" class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm focus:border-brand-green focus:outline-none focus:ring-1 focus:ring-brand-green">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Deskripsi Peristiwa</label>
                        <textarea name="deskripsi" required rows="5" placeholder="Tulis cerita sejarah lengkap peristiwa tersebut..." class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm focus:border-brand-green focus:outline-none focus:ring-1 focus:ring-brand-green"></textarea>
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-2 border-t border-slate-100">
                        ...
                        <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-slate-800 transition-colors cursor-pointer">Simpan Peristiwa</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto" x-show="showEditModal" x-cloak>
        <div class="flex min-h-screen items-center justify-center p-4 text-center">
            ...
            <div class="relative w-full max-w-xl rounded-2xl bg-white p-8 text-left shadow-xl border border-slate-100 transform transition-all">
                ...
                <form :action="'{{ route('admin.sejarah.index') }}/' + editItem.id" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Tahun Peristiwa</label>
                        <input type="text" name="tahun" required x-model="editItem.tahun" placeholder="Contoh: 1830 atau 1945" class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm focus:border-brand-green focus:outline-none focus:ring-1 focus:ring-brand-green">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Judul Peristiwa</label>
                        <input type="text" name="judul" required x-model="editItem.judul" placeholder="Contoh: Asal Usul Nama Desa" class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm focus:border-brand-green focus:outline-none focus:ring-1 focus:ring-brand-green">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Deskripsi Peristiwa</label>
                        <textarea name="deskripsi" required x-model="editItem.deskripsi" rows="5" placeholder="Tulis cerita sejarah lengkap peristiwa tersebut..." class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm focus:border-brand-green focus:outline-none focus:ring-1 focus:ring-brand-green"></textarea>
                    </div>
